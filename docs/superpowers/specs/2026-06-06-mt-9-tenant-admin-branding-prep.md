...
- локальный source Chatwoot CE `v4.13.0-1-g38c6b79b4`:
  ...
  - `../chatwoot-ce-stable/app/models/concerns/availability_statusable.rb`
  - `../chatwoot-ce-stable/swagger/tag_groups/application_swagger.json`
...
`access_token`. На странице Agents endpoint токен передается в header
`api_access_token`.
...
Официальная страница docs и официальный OpenAPI branch `develop` фиксируют:

- безопасность: `userApiKey`;
- `200`: массив active agents;
- `403`: access denied;
- agent fields включают `id`, `account_id`, `email`, `role`, `confirmed`,
  `availability_status`, `auto_offline`, `name`, `available_name`, `thumbnail`,
  `custom_role_id`;
- `availability_status` read-only и имеет enum `online`, `busy`, `offline`;
- enum роли включает `agent` и `administrator`.

Официальный OpenAPI tag `v4.13.0` отличается от текущей docs/develop:

- enum `availability_status` там описан как `available`, `busy`, `offline`;
- поле еще не помечено как read-only;
- описание поля менее точное.
...
- `app/views/api/v1/models/_agent.json.jbuilder` реально отдает
  `availability_status`, `auto_offline`, `confirmed`, `email`,
  `available_name`, `name`, `role`, `thumbnail`, а также `provider`,
  optional `custom_attributes` и enterprise-only `custom_role_id`;
- `AccountUser` в `v4.13` имеет enum availability `online`, `offline`, `busy`;
...
- официальная docs site, официальный OpenAPI branch `develop` и локальный runtime
  source `v4.13` используют `online`, `busy`, `offline`;
- официальный OpenAPI tag `v4.13.0` и bundled swagger в локальном `v4.13`
  описывают `availability_status` как `available`, `busy`, `offline`;
- admin-verification parser не должен зависеть от `availability_status` или
  `auto_offline`; эти поля нужно игнорировать для eligibility и использовать
  только `email`, `account_id`, `role` и `confirmed`.
...
- возвращает ли runtime `auto_offline`, `provider`, `custom_attributes` или
  другие дополнительные поля, которые parser должен игнорировать;
- какой `availability_status` enum возвращает конкретный Chatwoot instance:
  `online/busy/offline` по official docs/current runtime или
  `available/busy/offline` по official tag `v4.13.0` / bundled swagger;
...
- Chatwoot Agents response parser принимает поля из official/current docs,
  терпит дополнительные поля и не зависит от availability enum;
