...
- Backend принимает пробелы после запятых, но рекомендуемый формат - без
  пробелов.
- Backend принимает максимум `20` ID в одном person contact.
- Повторяющиеся ID backend схлопывает до одного lookup/thread entry с
  сохранением исходного порядка первого появления.
- Пустые элементы, не-integer значения, `0`, отрицательные числа, дробные
  значения и списки больше `20` ID являются configuration error и fail closed.
