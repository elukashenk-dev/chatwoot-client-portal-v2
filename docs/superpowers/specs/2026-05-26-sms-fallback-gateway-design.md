...
Status: deferred research/design artifact.

This document is preserved because SMS fallback has not been implemented yet and
the architecture notes can still be useful if the feature is reopened. It is not
current roadmap authority by itself. Before implementation, reopen this through
feature intake against the current offline-first PWA, chat runtime and production
operations baseline.
...
Historical docs branch:

```text
docs-sms-fallback-gateway-design
```

This branch existed for the original research/spec artifact. The current
preserved copy lives under `docs/superpowers/specs/` and must be refreshed
before any implementation branch is started.
