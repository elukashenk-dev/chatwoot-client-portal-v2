...
- [x] Add `isBackActionReadOnly?: boolean` to `ChatFullScreenPanel`,
      default `false`.
- [x] In normal mode keep current real back `<button>` unchanged.
- [x] In read-only mode render visually identical
      `<span aria-hidden="true">` with the chevron icon:
      no `onClick`, no `type`, no role, no focusability.
- [x] Add `isBackActionReadOnly?: boolean` to `ChatInfoPage` and pass it
      through to `ChatFullScreenPanel`.
- [x] Add/extend focused tests:
...
- [x] Implement `ChatInfoPreview`:
  ...
  - no API calls.
- [x] Wire `ChatInfoPreview` into `PortalPreviewFrame` for tab `Инфо`.
- [x] Add unit test:
...
- [x] Add a no-fetch assertion around switching to `Инфо`:
  - spy on `globalThis.fetch`;
  - click tab `Инфо`;
  - assert expected info content;
  - assert `fetchSpy` was not called.
- [x] For read-only back affordance tests, assert more than no button:
