...
findings: 9 Medium и 10 Low. Полный сгенерированный отчёт:
[Codex Security report](/tmp/codex-security-scans/chatwoot-client-portal-v2-audit-source-2026-07-13/a61b4975ae7b59e244c0b5bbc4efd02466aa075c_20260713T191131Z/report.md).
Его результаты импортированы в `F-SEC-001`–`F-SEC-019`; детали capability gate
и незавершённого Deep сохранены в [Stage 02](stages/02-security.md).
...
Сгенерированный security hardening portfolio предлагает две архитектурные
темы: generation-aware authority lifecycle и bounded work admission. Он
содержит варианты и trade-offs, но не заменяет прямые fixes:
[hardening portfolio](/tmp/codex-security-scans/chatwoot-client-portal-v2-audit-source-2026-07-13/a61b4975ae7b59e244c0b5bbc4efd02466aa075c_20260713T191131Z/hardening/hardening.md).
Практический порядок — сначала закрыть конкретные findings, затем отдельным
design scope решить, оправдана ли общая архитектурная граница.

## Unverified Areas And Blockers

Одиннадцать ledger rows остаются `needs_follow_up`; скрытых `candidate` или
`validating` строк нет.
...
| [F-OPS-005](../../../findings/F-OPS-005-deploy-authority-completion.md)      | `fix/ops-`          | deploy script tests + disposable deploy              | Один immutable artifact, migration-aware completion, bounded health gate и явный rollback decision. |
| [F-OPS-006](../../../findings/F-OPS-006-ssh-host-authentication.md)          | `fix/ops-`          | SSH negative/positive ops tests                      | Неизвестный/изменённый host key fail-closed; approved key проходит без interactive bypass.          |
...
| [F-DOC-001](../../../findings/F-DOC-001-source-of-truth-alignment.md)            | `docs/`         | Prettier, link/reference audit, `git diff --check` | Entry docs указывают только существующий branch/current next step; runtime gate не требуется. |
