...
- отделить актуальную архитектуру от старых transitional-решений;
...
- код из старого портала не переносится;
- старый портал не используется как reference;
- старые runtime-подходы не сохраняются ради совместимости.
...
- env values старого single-tenant вида допустимы только как bootstrap/dev input, а не как runtime source of truth.
...
- old v1 compatibility;
- перенос старых transitional layers.
...
- актуальная архитектура понятна без чтения старого портала.
