...
- `X-Forwarded-Host` учитывается только когда `PORTAL_TRUST_PROXY=true`, и этот режим допустим только если backend недоступен напрямую из интернета и получает трафик через контролируемый reverse proxy;
