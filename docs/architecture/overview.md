...
- своим доменом: custom client domain или provider-owned subdomain;
...
- production domain modes:
  - custom client domain: `lk.<client-domain>`;
...
Production domains can be provisioned in two ways:

- custom client domain: operator sets explicit `primary_domain` and
  `public_base_url`, for example `lk.buhfirma.ru` and
  `https://lk.buhfirma.ru`;
- provider-owned subdomain: operator supplies `--provider-subdomain=<slug>`,
  and backend derives `<slug>.<PORTAL_PROVIDER_TENANT_DOMAIN_SUFFIX>` plus
  `https://<slug>.<PORTAL_PROVIDER_TENANT_DOMAIN_SUFFIX>`.

In both modes the provider/operator owns tenant provisioning; public Chatwoot
signup does not create a production portal tenant.

## Runtime Flows

### Browser Flow

1. Browser приходит на tenant domain, например `lk.buhfirma.ru` или
