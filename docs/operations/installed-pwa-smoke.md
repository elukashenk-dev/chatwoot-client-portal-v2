...
| A4   | Войти online и открыть `private:me`.                                                                                                            | Чат загружен; на быстрой связи in-app startup screen не мелькает, на медленной связи показывается стабильно и без смены разных экранов.             |
...
| A7   | Запустить PWA с home screen offline.                                                                                                            | После предыдущего online входа открываются сохраненные tenant/auth/chat данные, вместо вечного splash; показывается один unified connection notice. |
...
| A10  | Вернуть сеть, пока PWA открыта.                                                                                                                 | Foreground drain отправляет queued message ровно один раз, статус queued исчез.                                                                     |
...
| A13  | Если браузер поддерживает one-off Background Sync: поставить message в очередь, закрыть PWA, вернуть сеть и дождаться platform background sync. | Сообщение может уйти в фоне; если platform не запустила sync, оно должно уйти при следующем open/online/visibility без дублей.                      |
...
| I4   | Войти online и открыть `private:me`.                                                                                                                                        | Чат загружен; на быстрой связи in-app startup screen не мелькает, на медленной связи показывается стабильно и без смены разных экранов.             |
...
| I7   | Запустить PWA с Home Screen offline.                                                                                                                                        | После предыдущего online входа открываются сохраненные tenant/auth/chat данные, вместо вечного splash; показывается один unified connection notice. |
...
| I10  | Вернуть сеть, пока PWA открыта.                                                                                                                                             | Foreground drain отправляет queued message ровно один раз, статус queued исчез.                                                                     |
...
На iOS/iPadOS не ожидать reliable background send, если PWA закрыта и сеть
вернулась позже. Ожидаемое поведение для queued text - отправка при следующем
open/online/visibility event без потери сообщения и без дублей.
