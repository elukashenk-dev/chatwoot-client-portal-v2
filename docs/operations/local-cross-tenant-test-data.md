...
- Chatwoot `contact` с `portal_contact_type=person` - это клиент, которому
  разрешена регистрация в портале.
- Portal `user` - запись в portal DB, создается после email-code registration
...
Portal registration и chat runtime зависят от Chatwoot contact data.

Для каждого portal test user должен существовать Chatwoot contact в правильном
Chatwoot account с exact email. Registration eligibility ищет contact по email
...
Если registration проходит, но чат возвращает `403 portal_contact_disabled`,
...
registration и mutating checks.

## Portal Registration Through Mailpit

Для полного LCT цикла не создавать portal users напрямую через
`pnpm --dir backend user:create`: этот shortcut не проверяет registration flow.

Правильный путь:

1. Chatwoot contact уже существует в correct account.
2. Portal registration request отправляет код в Mailpit.
3. Portal registration verify подтверждает код.
4. Portal registration set-password создает portal user и
   `portal_user_contact_link`.
5. Login идет через обычный `/api/auth/login` или UI.
...
2. Перейти в registration flow.
3. Ввести exact email Chatwoot person contact.
4. Открыть Mailpit: `http://127.0.0.1:8025`.
5. Найти письмо `Код подтверждения для Client Portal`.
6. Ввести 6-digit code.
7. Задать test password.
8. Войти в портал.

Если email не найден в Chatwoot account, registration request вернет
...
FULL_NAME="LCT Buhfirma ${RUN_ID}"
PASSWORD='Test1234!'

curl -sS -X POST "$TENANT_URL/api/auth/register/request" \
  -H "Origin: $TENANT_URL" \
  -H 'Content-Type: application/json' \
  --data "{\"email\":\"$EMAIL\",\"fullName\":\"$FULL_NAME\"}"
```
...
VERIFY_RESPONSE="$(
  curl -sS -X POST "$TENANT_URL/api/auth/register/verify" \
    -H "Origin: $TENANT_URL" \
    -H 'Content-Type: application/json' \
...
curl -sS -X POST "$TENANT_URL/api/auth/register/set-password" \
  -H "Origin: $TENANT_URL" \
  -H 'Content-Type: application/json' \
  --data "{\"email\":\"$EMAIL\",\"continuationToken\":\"$CONTINUATION_TOKEN\",\"newPassword\":\"$PASSWORD\"}"
```

Login:

```bash
curl -sS -i -X POST "$TENANT_URL/api/auth/login" \
  -H "Origin: $TENANT_URL" \
  -H 'Content-Type: application/json' \
...
2. Put the same group contact id in both person contacts'
   `portal_client_group_contact_ids`.
3. Register both users in the portal.
...
| ---------------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `REGISTRATION_CONTACT_NOT_FOUND`                           | Contact email is missing, typoed, or exists in another Chatwoot account | Create person contact in the current tenant Chatwoot account with exact email |
| Registration works, chat returns `portal_contact_disabled` | `portal_enabled` missing, false, or saved as string                     | Save boolean/checkbox `true` on person contact                                |
...
| Repeated auth/register requests get 429                    | Local auth rate limit                                                   | Wait for rate-limit window or use fewer repeated attempts                     |
...
- Mailpit registration success for every person user;
