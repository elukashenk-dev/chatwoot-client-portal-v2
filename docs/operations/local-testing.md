...
- Node.js 24.x;
- Docker Desktop;
- Chatwoot CE `v4.15.1`: `http://127.0.0.1:3000`;
...
### Chatwoot Нужно Запускать Полностью

Локальный Chatwoot состоит не из одного, а из трех обязательных процессов:

- `backend` (`Puma`) принимает API-запросы и открывает интерфейс агента;
...
Не использовать одиночный `bin/rails s -p 3000` как полный локальный запуск.
При работающем только Puma сообщение клиента сохраняется в Chatwoot, но
realtime-задача остается в Redis. В результате агент видит сообщение лишь
после обновления страницы.

Проверить версию и все три процесса:

```bash
curl -fsS http://127.0.0.1:3000/api
pgrep -af 'puma .*chatwoot-ce-stable|sidekiq .*chatwoot-ce-stable|vite.*development'
```

Ожидание:

- API возвращает `"version":"4.15.1"`;
- в списке есть Puma, Sidekiq и Vite;
- `queue_services:"ok"` само по себе недостаточно: эта строка подтверждает
  доступность Redis, но не наличие запущенного Sidekiq worker.
