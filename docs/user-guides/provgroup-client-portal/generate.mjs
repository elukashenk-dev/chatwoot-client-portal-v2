  ...
  await context.route('**/*', async (route) => {
    ...
    if (path.startsWith('/api/legal-documents/')) {
      ...
      await fulfillJson(route, {
        document: {
          content:
            'Демо-текст документа для пользовательской инструкции. Реальный документ загружается ответственным сотрудником.',
...
function supportVisual() {
  return `
    <div class="support-card">
      ...
      <p>Если код не приходит или войти не получается, свяжитесь с поддержкой. Мы проверим, почему вход не проходит, и поможем открыть личный кабинет клиента.</p>
  ...
  const pages = [
    page({
      body: `
        ...
        <p>На экране входа введите email, который вы используете для связи с PROVGROUP.</p>
        ${steps([
          'В адресной строке должен быть именно lk.provgroup.ru.',
          'После открытия вы увидите экран входа для клиентов.',
          'Введите email и перейдите к получению кода.',
        ])}
      `,
      kicker: 'Шаг 1',
    ...
    page({
      body: `
        <p>Основной вход работает по email-коду. Введите email, который вы используете для связи с PROVGROUP, и нажмите <strong>Получить код</strong>.</p>
        <p>Если пароль уже был настроен, можно перейти к входу по паролю отдельной ссылкой.</p>
        ${steps([
          'Введите ваш email для связи с PROVGROUP.',
    ...
    page({
      body: `
        ...
        ${steps([
          ...
          'После продолжения откроется чат. Если появится предложение установки, следующие шаги покажут, как его использовать.',
        ])}
      `,
      kicker: 'Шаг 4',
    ...
    page({
      body: `
        ...
        <p>Прочитайте и запомните шаги из подсказки. Кнопку <strong>Понятно</strong> можно нажать после того, как вы поняли порядок действий.</p>
        ${steps([
    ...
    page({
      body: `
        ...
        ${steps([
          'Данные профиля помогают проверить, что открыт ваш аккаунт.',
    ...
    page({
      body: `
        ...
        <p>Если личный кабинет клиента пишет, что войти не получается, свяжитесь с поддержкой. Мы проверим, почему вход не проходит, и поможем продолжить работу.</p>
      `,
      kicker: 'Шаг 11',
