...
- полное описание текущей архитектуры - оно живет в `docs/ARCHITECTURE.md`;
- порядок фаз и roadmap - они живут в `docs/IMPLEMENTATION_PLAN.md`;
- подробный multi-tenant design - он живет в
  `docs/MULTI_TENANT_PORTAL_ARCHITECTURE_PLAN.md`;
- список выполненных работ - он живет в `docs/WORK_LOG.md`;
...
- решение:
  ...
  records, contact links, conversation mappings, send ledger и webhook delivery
...
## D-009. Superseded: Portal chat использовал один primary conversation

- дата: `2026-04-21`
- статус: superseded by `D-019` on `2026-05-15`
- решение:
  клиентский чат портала строится как один authoritative `primary conversation`
...
- причина:
  ...
  pagination, realtime, idempotency и recovery.
- почему superseded:
  личный чат остается непрерывным, но продуктовая модель теперь поддерживает
  несколько доступных чатов: `private:me` и company threads. Поэтому Chatwoot
  conversation id больше не является публичной authority-моделью portal chat.
...
## D-017. Composer не содержит portal-owned emoji controls

- дата: `2026-04-26`
- решение:
  верхняя quick emoji лента, отдельная emoji-кнопка в composer и hardcoded
  emoji/preset picker не входят в текущий chat UX. Ввод emoji остается за
  системной клавиатурой или OS-level средствами пользователя.
- причина:
  кастомные emoji controls загромождают composer на мобильных экранах и
  дублируют системный ввод, особенно в iOS/PWA-контексте.
...
- причина:
  один `primaryConversationId` подходил для одного личного чата, но плохо
  масштабировался на общий чат компании. Portal-owned `threadId` сохраняет
  backend authority, не раскрывает Chatwoot IDs браузеру как право доступа и
  позволяет добавлять личный и company-чаты без отдельной portal admin panel.
