...
- Реализован `MT-7 Tenant-Aware Webhooks And Provisioning`: снят transitional hard-fail для non-default customer runtime, webhook signature проверяется tenant secret по Host, добавлен tenant-aware webhook configure script с encrypted secret update в `portal_tenants`; backend build/lint/test, code-health, Prettier и `git diff --check` пройдены.

## Recommended Next Step

- Сделать checkpoint commit для `MT-7 Tenant-Aware Webhooks And Provisioning`; затем перейти к `MT-8 Tenant-Aware Frontend/PWA`.
