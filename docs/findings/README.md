...
- Закрытие finding фиксировать в `docs/roadmap/work-log.md` только если оно
  меняет устойчивый product/architecture/runtime baseline.
- Не писать в `docs/roadmap/work-log.md` команды, счетчики тестов, списки
  проверок и технические детали закрытия finding; эти детали остаются в
  финальном ответе, commit/PR description или finding-файле, пока finding открыт.
